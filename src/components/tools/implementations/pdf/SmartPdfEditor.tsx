...
export function SmartPdfEditor() {
  ...
  // Clean up PDF.js proxy when unmounting or resetting
  ...
  const loadDocument = useCallback(
    async (bytes: Uint8Array, docFilename: string, docSize: number) => {
      ...
      try {
        ...
        // 2. Load into PDF.js for canvas rendering (copy bytes buffer to prevent detached ArrayBuffer)
        ...
        // 3. Initialize field values dictionary
  ...
  const handleFitWidth = useCallback(() => {
    ...
    const containerWidth = viewportContainerRef.current.clientWidth - 64; // subtract padding
