...
fuzz_target!(|data: &[u8]| {
    ...
    let rect = [0.0, 0.0, f64::from(data[0]).max(1.0), f64::from(data[1]).max(1.0)];
    let da = DefaultAppearance::default();
    let comb = usize::from(data[2]).min(64);
    let _ = TextFieldAppearance::generate_stream_with_options(rect, &text, &da, i32::from(data[3] % 3), TextLayoutOptions {
    ...
    let selected = if options.is_empty() { Vec::new() } else { vec![0] };
